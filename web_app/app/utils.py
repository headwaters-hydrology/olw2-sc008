...
dataset_ids = {'velocity': 'a66a69063e43457e905de6a8',
...
def calc_hs(data, hsc_dict, species, refs):
    """

    """
    # data = data.dropna('time')
    dims = data['gage_height'].dims
    gh_data = data['gage_height'].values
    vel_data = data['velocity'].values
    dtype = vel_data.dtype

    hsc_dict1 = hsc_dict[species]

    for ref, xy in hsc_dict1.items():
        if ref in refs:
            gh_hs = np.interp(gh_data, **xy['gage_height'])
            vel_hs = np.interp(vel_data, **xy['velocity'])
            hs = (gh_hs * vel_hs).astype(dtype)
            data[ref] = (dims, hs)
